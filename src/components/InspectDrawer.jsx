...
const InspectDrawer = ({ panelID, setDrawerOpen, drawerOpen, dataPanelID }) => {
  ...
  const { dataDetail, data, dataTable, chart_data } = useSelector(
    (state) => {
      ...
      return {
        ...
        chart_data: targetPanel[0]?.chart_data[0]?.dataTable, ////////////[0]??
      };
    }
  );

  let inspectValue;
  if (pathname === "/" && chart_data !== undefined) {
    inspectValue = JSON.stringify(chart_data, null, 2);
  } else if (pathname === "/" && chart_data === undefined) {
