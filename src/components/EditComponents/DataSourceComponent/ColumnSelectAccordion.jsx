...
const ColumnSelectAccordion = ({ panelID, data }) => {
  ...
  const contain_error = data.columnSelect.map((colKeys, colIdx) => {
    try {
      return !Object.keys(data.dataDetail[0]).includes(colKeys[0])
      || columnNameText.filter(n => n === columnNameText[colIdx]).length > 1
      || columnNameText[colIdx] === ''
    } catch {
      return true
    }
  })
  ...
  const onChangeColName = (e, col_index) => {
    ...
    let tmp = JSON.parse(JSON.stringify(data.columnName))
  ...
  return data.dataDetail?.length > 0 && data.acceptType ? (
    <Accordion
      ...
      sx={{
        mt: 2,
        overflow: 'auto',
        border: contain_error.includes(true) ? 1 : null,
        borderColor: contain_error.includes(true) ? 'red' : null,
      }}
    >
