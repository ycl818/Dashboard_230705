...
const StreamingSourceBlock = ({ panelID }) => {
  ...
  const fetchURl = async (variablesArray, currentText, dataPanelID) => {
    try {
      ...
      console.log(
        "🚀 ~ file: StreamingSourceBlock.jsx:100 ~ fetchURl ~ error:",
        error
      );
  ...
  const [debounceURl, setDebounceURL] = useState("");
  const [targetDebouncePanel, setTargetDebouncePanel] = useState("");
...
export default StreamingSourceBlock;
